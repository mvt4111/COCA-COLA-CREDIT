...
st.markdown(
    ...
    .ledger-table .paid-row td {
        background-color: #DCFCE7;
        color: #166534;
    }

    .ledger-table .bill-code {
        font-weight: 800;
        color: #111827;
        white-space: nowrap;
    }

    .ledger-table .manager-name {
        font-weight: 600;
        color: #334155;
    }

    .ledger-table .outlet-name {
        font-weight: 800;
        color: #111827;
    }

    .ledger-table .balance-amount {
        font-weight: 800;
        color: #DC2626;
        white-space: nowrap;
    }

    .ledger-table .paid-balance {
        font-weight: 800;
        color: #166534;
        white-space: nowrap;
    }

    .ledger-table .paid-amount {
        font-weight: 700;
        color: #16A34A;
        white-space: nowrap;
    }

    .ledger-table .status-paid {
        color: #166534;
        font-weight: 800;
    }

    .ledger-table .status-partial,
    .ledger-table .status-unpaid {
        color: #DC2626;
        font-weight: 800;
    }

    .ledger-table .dues-days {
        color: #DC2626;
        font-weight: 800;
        white-space: nowrap;
    }

    .ledger-table .zero-days {
        color: #166534;
        font-weight: 700;
        white-space: nowrap;
    }
...
def save_bill_to_db(
    ...
    bill_no = generate_auto_code_backend(
        outlet
    )
...
with col_right:
    ...
    if not outlets_list:
        ...
        if unpaid_bills_df.empty:
            ...
            st.success(
                f"🎉 No outstanding unpaid "
                f"bills for {p_outlet}!"
            )
...
with tab_bills:
    ...
    if bills_df.empty:
        ...
        # COLUMN 3 = DATE
        # COLUMN 4 = OUTLET
        # COLUMN 5 = BILL AMOUNT
        # COLUMN 6 = PAID AMOUNT
        # COLUMN 7 = BALANCE
        # COLUMN 8 = STATUS
        # COLUMN 9 = DUES DAYS
        #
        ...
        for _, row in df_view.iterrows():
            ...
            <tr class="{row_class}">
                ...
                <td>
                    Rs {float(row['Bill_Amount']):,.2f}
                </td>

                <td>
                    <span class="paid-amount">
                        Rs {float(row['Paid_Amount']):,.2f}
                    </span>
                </td>
        ...
        rep_sub = (
            f"Outlet: "
            f"{selected_outlet_filter} | "
            f"Manager: "
            f"{selected_mgr_filter}"
        )
            ...
            if not wa_phone.strip():
                ...
                for _, wa_row in (
                    ...
                    msg += (
                        ...
                        f"💰 *Bill Amount:* "
                        f"Rs "
                        f"{float(wa_row['Bill_Amount']):,.2f}\n"
                        f"🟢 *Paid Amount:* "
                        f"Rs "
                        f"{float(wa_row['Paid_Amount']):,.2f}\n"
                        f"🔴 *Balance Due:* "
                        f"Rs "
                        f"{float(wa_row['Balance']):,.2f}\n"
                        f"📌 *Status:* "
                        f"{wa_status}\n"
                        f"⏰ *Dues Days:* "
                        f"{wa_dues_days} Days\n"
                ...
                if st.session_state.get(
                    ...
                    st.markdown(
                        ...
                        <div class="delete-card">
                            ...
                            <p style="margin:5px 0 0 0;">
                                Delete payment of
                                <b>Rs {float(p_row['Amount_Paid']):,.2f}</b>
                                for Bill
                                <b>{p_row['Bill_No']}</b>?
                                <br>
            ...
            if st.button(
                ...
                if rec_wa_phone.strip():
                    ...
                    msg = (
                        ...
                        f"🟢 *AMOUNT RECEIVED:* "
                        f"Rs "
                        f"{float(latest_pay['Amount_Paid']):,.2f}\n"
                        f"💳 *Payment Mode:* "
