...
def update_bill_in_db(
    ...
    if amount < paid_amount:
        ...
        return (
            False,
            f"Bill amount cannot be less than already paid amount Rs {paid_amount:,.2f}.",
        )
...
def generate_pdf_report(
    ...
    # --------------------------------------------------------------------------
    # HEADERS (Dues Days set to Second Last as requested)
    # --------------------------------------------------------------------------
    headers = [
        "Bill Code",
        "Date",
    ...
    if not outlets_list:
        ...
        if unpaid_bills_df.empty:
            ...
            for _, row in unpaid_bills_df.iterrows():
                # Dues days removed from payment selection interface as requested
                label = (
                    f"{row['Bill_No']} | "
                    f"Date: {row['Date']} | "
                    f"Balance Due: "
